...
func (a *accountsHandler) handleGetAccountTransactions(ctx context.Context, req *models.AccountTransactionsRequest) ([]models.AccountTransactionsResponse, error) {
	...
	for results.Next() {
		...
		err = results.Scan(&account.AccountID, &account.TransactionID, &account.Amount, &account.Sendreceiveflag, &account.SenderID, &account.ReceiverID, &account.Timestamp, &account.Status)
		if err != nil {
