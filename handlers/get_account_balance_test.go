...
func TestValidateGetAccountBalanceRequest(t *testing.T) {
	type fields struct {
		logger     *slog.Logger
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			app := fiber.New()
			app.Get("v1/accounts/:id", a.GetAccountBalance)

			param := fmt.Sprintf("https://localohost:8080/v1/accounts/%s", test.accountID)
